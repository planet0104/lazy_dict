...
impl Context {
    ...
    {
        trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 001");
        if opengl.sharing.is_some() {
            trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 101");
            unimplemented!()
        }
        trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 002");

        // calling `eglGetDisplay` or equivalent
        let display = get_native_display(&egl, native_display);
        trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 003");

        if display.is_null() {
            trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 102");
            return Err(CreationError::OsError("Could not create EGL display object".to_string()));
        }
        trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 004");
        let egl_version = unsafe {
            trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 0041");
            let mut major: ffi::egl::types::EGLint = mem::uninitialized();
            let mut minor: ffi::egl::types::EGLint = mem::uninitialized();

            if egl.Initialize(display, &mut major, &mut minor) == 0 {
                trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 00411");
                return Err(CreationError::OsError(format!("eglInitialize failed")))
            }
            trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 0042");

            (major, minor)
        };
        trace!("new>>>>>>>>>>>>>>>>>>>>>>>>>>>egl Context 005");
...
impl<'a> ContextPrototype<'a> {
    ...
    {
        trace!("ContextPrototype>>>finish 001");
        let surface = unsafe {
            trace!("ContextPrototype>>>finish 100");
            let surface = self.egl.CreateWindowSurface(self.display, self.config_id, native_window,
                                                       ptr::null());
            trace!("ContextPrototype>>>finish 101");
            if surface.is_null() {
                trace!("ContextPrototype>>>finish 110");
                return Err(CreationError::OsError(format!("eglCreateWindowSurface failed")))
            }
            trace!("ContextPrototype>>>finish 102");
            surface
        };
        trace!("ContextPrototype>>>finish 002");
