...
/// - `src`: 图像数据.
/// - `out`: 输出(背景为白色，前景为黑色)
/// - `thresholds`: 多个阈值 0~255
/// - `bpp`: 每个像素占用字节 
pub fn edge_detect(width:usize, bpp: usize, src:&[u8], out:&mut [u8], threshold:u8){
    let bpl = width*bpp;//每行字节
    ...
    let mut i = 0;
    for pixel in src.chunks(bpp){
        let hrid = i+bpp;//右边一个像素[水平细胞]的位置(向后偏移一个像素)
        let hbid = i+bpl;//下边一个像素[水平细胞]位置(向后偏移一行)

        //当前像素[双极细胞]输出     -- 亮光兴奋，弱光抑制
        let b_out = if 0.299*pixel[0] as f32 + 0.587*pixel[1] as f32 + 0.114*pixel[2] as f32 >= threshold{
            1.0
        }else{
            -1.0
        };
        ...
        if hrid<total_bytes && hbid < total_bytes{
            let hr_out = if 0.299*src[hrid] as f32 + 0.587*src[hrid+1] as f32 + 0.114*src[hrid+2] as f32 >= threshold{
                -1.0
            }else{
                1.0
            };
            let hb_out = if 0.299*src[hbid] as f32 + 0.587*src[hbid+1] as f32 + 0.114*src[hbid+2] as f32 >= threshold{
                -1.0
            }else{
                1.0
            };
